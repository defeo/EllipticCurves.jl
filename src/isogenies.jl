...
export degree, rational_fractions, areisomorphic, compose, squarefree_kernel, isomorphism_coefficients, subgroup, dual, isomorphism, find_isomorphism, multiplication_isogeny
...
"""
#TODO: make this a Nemo.Map
mutable struct Isogeny{T}
	...
	x::Union{Frac{U}, Nothing} where U <: PolyElem{T}
	y::Union{Frac{U}, Nothing} where U <: PolyElem{T}
end
...
"""
function Isogeny(E::AbstractWeierstrass{T}, poly::PolyElem{T}, Eprime::AbstractWeierstrass{T}) where T
	phi1 = Isogeny(E, poly)
...
function Isogeny(E1::AbstractWeierstrass{T}, E2::AbstractWeierstrass{T}, phix::Frac{U}, phiy::Frac{U}) where U<:PolyElem{T} where T 
	...
	res = Isogeny(E1, 2 * degree(K) + 1, K, u, F(0), F(0), F(0), E2, phix, phiy)
	@assert isvalid(res)
	return res
end
...
"""
function Isogeny(E::AbstractWeierstrass{T}, poly::PolyElem{T}) where T
	...
	return Isogeny(E, 2 * n + 1, poly, K(1), K(0), K(0), K(0), E1, nothing, nothing)
end
...
"""
function subgroup(Q::EllipticPoint{T}, l::Nemo.Integer) where T<:Nemo.FieldElem
	normalize!(Q)
	K = base_ring(Q)
...
function Isogeny(E::ShortWeierstrass{T}, degree::Nemo.Integer, jprime::T; USE_ATKIN=false) where T
	K = base_ring(E)
	j = j_invariant(E)
	...
	if USE_ATKIN
		...
		return Isogeny(E, degree, poly, K(1), K(0), K(0), K(0), Eprime, nothing, nothing)
	end
end
...
"""
function Isogeny(E::ShortWeierstrass{T}, degree::Nemo.Integer, jprime::T, f::T) where T
	j = j_invariant(E)
...
function Isogeny(E::AbstractWeierstrass{T}, Q::EllipticPoint{T}, l::Nemo.Integer) where T<:Nemo.FieldElem
	poly = subgroup(Q, l)
...
function Isogeny(E1::AbstractWeierstrass{T}, degree::Nemo.Integer, E2::AbstractWeierstrass{T}) where T<:Nemo.FieldElem
...
function isomorphism(E::AbstractWeierstrass{T}, u::T) where T
	...
	phi = Isogeny(E, 1, R(1), u, K(0), K(0), K(0), EllipticCurve(a2prime, a4prime, a6prime), nothing, nothing)
	return phi
end

function find_isomorphism(E1::AbstractWeierstrass{T}, E2::AbstractWeierstrass{T}) where T
